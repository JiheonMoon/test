...
--FLOOR() 주어진 숫자보다 작거나 같은 정수중 최대값 반환
...
--POWER() 주어진 숫자의 지정된 수만큼 제곱
SELECT POWER(2,1), POWER(2,2), POWER(2,3), POWER(2,0) FROM DUAL;

--사원 테이블에서 사원번호가 짝수인 사람은 짝수인 사람은 0, 홀수인 사람은 1을 사원번호 연산결과 순으로 조회
SELECT EMPLOYEE_ID, MOD(EMPLOYEE_ID, 2) FROM EMPLOYEES;

--사원 번호가 짝수인 사람들의 사원번호와 이름을 조회
SELECT EMPLOYEE_ID, FIRST_NAME
FROM EMPLOYEES
WHERE MOD(EMPLOYEE_ID, 2) = 0
ORDER BY EMPLOYEE_ID;

--사원 테이블에서 이름, 급여, 급여의 1000당 ■로 채워 조회하세요
--EX 급여 8000이면 ■■■■■■■■
...
--NEXT_DAY() 주어진 날짜 다음에 나타나는 지정요일(1일요일 ~ 7토요일)
...
--NVL() NULL일때 치환할값, NULL이 아닐때 치환할값
SELECT EMPLOYEE_ID, FIRST_NAME, NVL2(COMMISSION_PCT,1,0)
FROM EMPLOYEES;

--순위 함수
--RANK() OVER() 그룹 내 순위를 계산하여 NUMBER타입으로 순위를 반환
...
--집계함수
--여러행들에 대한 연산결과를 하나의 행으로 반환
--집계함수는 NULL을 체크하지 않는다
--평균을 구할때 내가 원하는 값과는 다른 결과가 나올수 있음
...
--사원테이블에서 직종이 'SA_REP'인 사원들의 평균급여, 최고급여, 최저급여, 급여의 총합을 조회하세요
...
--GROUP BY 그룹화 특정 테이블에서 소그룹을 만들어 결과를 분산시켜 얻고자 할때
...
--부서별, 직종별로 그룹을 나눠 인원수를 출력하되, 부서번호가 낮은순으로 정렬
...
--GROUPING SETS 특정항목에 대한 소계를 내는 함수
SELECT 상품ID, 월, SUM(매출액) FROM 월별매출
GROUP BY GROUPING SETS(상품ID,월);

--각 부서의 최대, 최소급여, 인원수를 출력하되, 급여의 합이 8000이상인 결과만 조회
SELECT DEPARTMENT_ID, MAX(SALARY), MIN(SALARY), COUNT(*) FROM EMPLOYEES
GROUP BY DEPARTMENT_ID
HAVING SUM(SALARY) >= 8000;

--각 부서별 인원수가 20명이상인 부서의 정보를 부서번호, 급여의 합, 급여의 평균, 인원수 순으로 출력
...
--부서별, 직종별로 그룹화 하여 결과를 부서번호, 직종, 인원수 순으로 출력하되, 직종이 'MAN'으로 끝나는 경우에만 조회하세요
SELECT DEPARTMENT_ID, JOB_ID, COUNT(*) 
FROM EMPLOYEES
WHERE JOB_ID LIKE '%MAN'
GROUP BY DEPARTMENT_ID, JOB_ID;
--HAVING JOB_ID LIKE '%MAN' 도 가능

--각 부서별 평균 급여를 소수점 한자리까지 버림으로 조회하되,
--평균급여가 10000미만인 그룹만 조회해야 하며
--부서번호가 50 이하인 부서만 조회
...
-- 이름이 'Michael'이고, 직종이 'MK_MAN'인
-- 사원의 급여보다 많이 받는 사원들의 정보를 사번, 이름 직종, 급여순으로 출력
SELECT SALARY
FROM EMPLOYEES e 
WHERE FIRST_NAME = 'Michael' AND JOB_ID = 'MK_MAN';
--13000보다 만이 받는 사원들의 종보를 사번, 이름, 직종, 급여순으로
...
--사원번호가 150번인 사원의 급여와 같은 급여를 받는 사원들의 정보를 사번, 이름, 급여순으로 출력하세요

...
--EMP 테이블에서 SAL이 3000이상이면 HIGH, 1000이상이면 MID, 둘다 아니면 LOW를 
--사원명(ENAME), 급여(SAL), 등급(GRADE)순으로 조회하세요
...
--STADIUM 테이블에서 SEAT_COUNT 좌석수가 0이상 30000이하면 'S',
...
--71이상 80이하면 'M' NULL이면 '미등록' 이외에는 'H'
--선수이름, 몸무게(단위붙이기), 크기 순으로 조회
SELECT PLAYER_NAME AS 이름, WEIGHT||'kg' AS 몸무게,
...
IS
BEGIN
	DBMS_OUTPUT.PUT_LINE('X : '|| X ||', Y : '|| (X*2 + 1));
END;

CALL F(5);

SELECT COUNT(*) FROM JOBS WHERE JOB_ID = 'IT';
...
--호출했을때 4개의 값을 전달받아서 JOBS에 INSERT할것
...
--INSERT기능을 하는 프로시저에서 PK 제약조건으로 인해 겹칠때까 있다.
--데이터가 겹치게 되면 오류를 내는 것이 아니라 UPDATE통해 내용을 수정하자

--삭제를 하는 DEL_JOB_PROC만들기
--JOB_ID에 들어가는 값은 매개변수를 통해 받는다
CREATE OR REPLACE PROCEDURE DEL_JOB_PROC(
	D_JOB_ID IN JOBS.JOB_ID%TYPE
)
IS CNT NUMBER := 0;
BEGIN 
	SELECT COUNT(JOB_ID) INTO CNT FROM JOBS
	WHERE JOB_ID = D_JOB_ID;
	IF CNT !=0 THEN
		DELETE FROM JOBS
		WHERE JOB_ID = D_JOB_ID;
		DBMS_OUTPUT.PUT_LINE(D_JOB_ID || '가 삭제되었습니다');
	ELSE
...
--시퀀스
--테이블에 값을 추가할때 자동으로 순차적인 정수값이 들어가도록 설정해두는 객체
